...
the literature fits what you want to do, mark it `ESTIMATE`, give it low
confidence, and say so. Don't invent constants and present them as science.
...
repo rather than committed to it. Those tests skip automatically when the
capture is absent, and CI runs the same way. Everything else runs everywhere.
...
Working out an undecoded field, opcode, or event is one of the most valuable
contributions you can make. Open an issue in
